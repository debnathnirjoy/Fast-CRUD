...
def create_user(db: Session, user: UserCreateSchema) -> UserRetrieveSchema:
    ...
    new_user = user_repository.create(db, user)
    if new_user is None:
...
def update_user_by_id(db: Session, user_id: str, updated_user: UserUpdateSchema):
    update_count = user_repository.update_by_id(db, user_id, updated_user)
    if update_count > 0:
...
def delete_user_by_id(db: Session, user_id: str):
    delete_count = user_repository.delete_by_id(db, user_id)
    if delete_count > 0:
